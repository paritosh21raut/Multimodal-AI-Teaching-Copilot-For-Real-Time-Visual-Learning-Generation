"""
Slide Renderer (FIXED)

Fixed: Hero definition accent line stays within bounds.
"""
...
class SlideRenderer:
    ...
    def _render_title(self, slide, plan, layout):
    ...
    def _render_content(self, slide, plan, layout, visual_spec):
    ...
    def _render_hero_definition(self, slide, plan, layout):
        """Hero definition: centered definition text with accent line"""
        ...
        # Accent line - FIXED: stays within slide bounds
        accent_width = 0.35 * design_system.slide_width
        accent_left = (design_system.slide_width - accent_width) / 2
        accent_top = (top + height + 0.02) 
        accent_height = 0.015
        
        # Ensure accent stays within slide
        if accent_top + accent_height < design_system.slide_height - 0.1:
    ...
    def _render_horizontal_flowchart(self, slide, nodes, left, top, width, height):
        if not nodes:
            return
    ...
    def _render_table(self, slide, columns, rows, left, top, width, height):
        if not columns:
            return
    ...
    def _render_two_column_contrast(self, slide, plan, layout, visual_spec):
        ...
        self._render_text_column(slide, plan, left_region)
        self._render_text_column(slide, plan, right_region)
    
    def _render_text_column(self, slide, plan, region):
    ...
    def _render_centered_formula(self, slide, plan, layout):
        ...
        paragraph = text_frame.paragraphs[0]
        paragraph.text = plan.focal_message
