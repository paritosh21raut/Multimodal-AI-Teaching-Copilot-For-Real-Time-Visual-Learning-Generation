"""
Phase 7: Presentation Planner - Production Readiness Tests
"""
...
from app.presentation.models.presentation_models import (
    ...
    RepresentationType, LayoutFamily, ContentBlockType,
)
...
def create_planner_inputs():
    ...
    info = SelectedInformation(
        focal_claim="TCP provides reliable delivery",
        semantic_units=["TCP PROVIDES reliability"],
        definitions=["TCP"],
        evidence_ids=["e1"],
    )
...
def test_planner_generic_domains():
    ...
    for domain, claim in domains.items():
        info = SelectedInformation(focal_claim=claim, semantic_units=[claim])
        ...
        assert plan.focal_message == claim, f"Failed for {domain}"